## LLM & Transformer

### Books

- **Deep Learning Bible - NLP** (wikidocs)  
    - https://wikidocs.net/book/7100

- **Deep Learning Bible - LLM** (wikidocs)  
    - https://wikidocs.net/book/14965

- **LLM from scratch** (Sebastian Raschka)  
    - https://github.com/rasbt/LLMs-from-scratch

### Lectures

- **스탠포드 Transformer & LLM 강의**    
    - https://www.youtube.com/watch?v=Ub3GoFaUcds

- **워털루 Foundation Models 강의**
    - https://cs.uwaterloo.ca/~wenhuche/teaching/cs886/

- **프린스턴 LLM 강의자료**
    - https://www.cs.princeton.edu/courses/archive/fall22/cos597G/

- **하버드대 Machine Learning System 교재**
    - [ML Systems Textbook](https://www.mlsysbook.ai/)

### Paper / Research / References

- **Transformer study** ("A survey of Transformer" Survey 논문 base, DSBA 연구실)   
  - https://github.com/yukyunglee/Transformer_Survey_Study

- **Awesome-LLM** (trending paper, researches)  
  - https://github.com/Hannibal046/Awesome-LLM

### Articles

- **Finetuning LLMs with Reinforcement Learning** - 강화학습을 통한 LLM 파인튜닝    
    - https://medium.com/data-science-at-microsoft/fine-tuning-llms-with-reinforcement-learning-ef84fe42d6a6

- **multimodal LLM** (Sebastian Raschka)    
    - https://magazine.sebastianraschka.com/p/understanding-multimodal-llms

- **2024 LLM paper list** (Sebastian Raschka)   
  - https://magazine.sebastianraschka.com/p/llm-research-papers-the-2024-list

### Leaderboards

- **Function Calling leaderboard** : function calling → BFCL Score가 높은 베이스 모델 사용하기
  - https://gorilla.cs.berkeley.edu/leaderboard.html

- **Embedding Leaderboard (MTEB)**  
  - https://huggingface.co/spaces/mteb/leaderboard

- **Chatbot Arena leaderboard (LMArena)**   
  - https://lmarena.ai/leaderboard 
  - (or https://huggingface.co/spaces/lmarena-ai/lmarena-leaderboard)


---

## DL (Deep Learning)

- **Understanding Deep Learning**   
  - https://udlbook.github.io/udlbook/
---

## RL (Reinforcement Learning)

- **Deep learning Bible** (Wikidocs) 
https://wikidocs.net/book/7888
    - GitHub Repository : Reinforcement Learning - OpenAI Gym [[Click Here](https://github.com/RichardMinsooGo-RL-Gym)]
    - GitHub Repository : Reinforcement Learning - Single Agent [[Click Here](https://github.com/RichardMinsooGo-RL-Single-agent)]
    - GitHub Repository : Reinforcement Learning - Multi Agent [[Click Here](https://github.com/RichardMinsooGo-RL-Multi-agent)]

- **해외 대학들 강화학습 강의모음**     
  - https://helpingstar.github.io/rl/RLlecture/

- **고려대학교 오승상 교수님 강화학습 강의**    
  - https://www.youtube.com/playlist?list=PLvbUC2Zh5oJtYXow4jawpZJ2xBel6vGhC

- **Deep Reinforcement Learning** (Springer)    
  - https://arxiv.org/pdf/2201.02135

- **Reinforcement Learning: An Overview**   
  - https://arxiv.org/pdf/2412.05265

- **RL 방법론 모음집**  
  - https://github.com/aikorea/awesome-rl 
  - https://github.com/kristery/Awesome-Imitation-Learning.git

- **multi-agent 모음집**    
  - https://github.com/kyegomez/awesome-multi-agent-papers
  - https://github.com/WeiChengTseng/awesome-multi-agent

---

## Robotics

- **Awesome-LLM-Robotics** (paper, researches)  
    - https://github.com/GT-RIPL/Awesome-LLM-Robotics

- **Leisaac, Lerobot**  
  - https://github.com/LightwheelAI/leisaac?tab=readme-ov-file
  - https://github.com/huggingface/lerobot

### Isaac Sim / Isaac Lab

- **Mobile Robot Manipulation** (Isaac Sim)
  - https://youtu.be/k4R60XoZmIE?si=IJuPMmqH4BDnAi5F


## World Model 

- awesome-world-models
  - https://github.com/leofan90/Awesome-World-Models

---

## Conference Deadlines

- **AI Deadlines**
  - http://aideadlines.org/?sub=ML,CV,NLP,RO,SP,DM,AP,KR,HCI,IRSM,MISC